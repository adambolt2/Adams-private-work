...
// Create an array to store your friends' user IDs
$yourFriends = [];
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        if ($row['user_id_1'] == $MyID) {
            $yourFriends[] = $row['user_id_2'];
        } else {
            $yourFriends[] = $row['user_id_1'];
        }
    }

    // Remove duplicates by converting the array to a set and back to an array
    $yourFriends = array_values(array_unique($yourFriends));
}

// Create an array to store friends of friends (excluding yourself)
$friendsOfFriends = [];

// Loop through each of your friends
// Loop through each of your friends
foreach ($yourFriends as $friendId) {
    // Prepare a query to get friends of each of your friends (excluding yourself and your friends)
    $sql = "SELECT DISTINCT user_id_1, user_id_2 FROM network WHERE (user_id_1 = ? OR user_id_2 = ?) 
            AND user_id_1 != ? AND user_id_2 != ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssss", $friendId, $friendId, $MyID, $friendId);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Check which user_id matches $MyID and store the other user_id
            if ($row['user_id_1'] == $friendId) {
                $potentialFriend = $row['user_id_2'];
            } else {
                $potentialFriend = $row['user_id_1'];
            }

            // Check if this potential friend is not already your friend
            if (!in_array($potentialFriend, $yourFriends)) {
                $friendsOfFriends[] = $potentialFriend;
            }
        }
    }
}
...
// Remove duplicates by converting the array to a set and back to an array
$friendsOfFriends = array_values(array_diff(array_unique($friendsOfFriends), [$MyID]));
// Create an array to store sender information
$senderInfoArray = [];

// Loop through each user ID
foreach ($friendsOfFriends as $senderId) {
    $senderInfo = [];
    ...
    $posts = [];
    $twentyFourHoursAgo = date('Y-m-d H:i:s', strtotime('-1000 hours'));

    // Modify your query to select posts in the last 24 hours
    $postsQuery = "SELECT post_id, post_image, post_desc, post_time FROM posts WHERE user_id = ? AND post_time >= ? ORDER BY post_time DESC";    
    $stmtPosts = $conn->prepare($postsQuery);
    $stmtPosts->bind_param("ss", $senderId, $twentyFourHoursAgo);
    $stmtPosts->execute();
